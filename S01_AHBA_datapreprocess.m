...
%remap = readtable('AllenInstitute_custom_Agilent_Array.txt');%https://sourceforge.net/projects/reannotator/
...
%AHBA proble info
FileProbes = './rawdata/normalized_microarray_donor9861/Probes.csv';
...
headerprobe = { 'ProbeName', 'EntrezID', 'GeneSymbol','ProbeID'}; %

%Go to each subject's directory and take the data
donor = [9861,10021,12876,14380,15496,15697];

cd %consult current working directory
for subj=1:6
    folder = sprintf('./rawdata/normalized_microarray_donor%d', donor(subj))
    cd(folder);

    %%load information specific for each subject
    FileMicroarray = 'MicroarrayExpression.csv';
    FileAnnot = 'SampleAnnot.csv';
    ...
    SlabType(any(cellfun(@(x) any(isnan(x)), SlabType),2),:) = [];  %exclude null rows of loded csv file
...
[~,ia,ib] = intersect(Uniq,EntrezID_s);
...
v1 = spm_vol('./brain_atlas/HarvardOxford-cort-maxprob-thr25-2mm_YCG.nii');
% v1 = spm_vol('./brain_atlas/Harvard-Oxford cortical and subcortical structural atlases/HarvardOxford-cort-maxprob-thr25-1mm.nii');
[cort, cort_XYZ] = spm_read_vols(v1);%91*109*91
...
v2 = spm_vol('./brain_atlas/HarvardOxford-sub-maxprob-thr25-2mm_YCG.nii');
% v2 = spm_vol('./brain_atlas/Harvard-Oxford cortical and subcortical structural atlases/HarvardOxford-sub-maxprob-thr25-1mm.nii');
[sub, sub_XYZ] = spm_read_vols(v2);
...
%     voxel_index = inv(v1.mat)*[MNI_x(i);MNI_y(i);MNI_z(i);1];
...
ROI_radius = 4;
len = size(Coordinatesall,1);
sample_roi = zeros(len,3);%first column is probe ID, second column is ROI ID.
B_cort = cort_XYZ';
B_sub = sub_XYZ';

for i = 1:len
    A = Coordinatesall(i,:);
    distance_cort = sqrt(sum((B_cort-repmat(A,size(B_cort,1),1)).^2,2));% calculate distance of sample A and all point in B
    distance_sub = sqrt(sum((B_sub-repmat(A,size(B_sub,1),1)).^2,2));% calculate distance of sample A and all point in B
    sample_roi(i,1) = i;
    temp = find(distance_cort<ROI_radius);
    sample_roi(i,2) = size(find(cort(temp)~=0),1)/size(temp,1);

    temp = find(distance_sub<ROI_radius);
    sample_roi(i,3) = size(find(sub(temp)~=0),1)/size(temp,1);
...
overlap_rate = 0.8;
Ncort_l_80 = [find(sample_roi(:,2)>=overlap_rate),sample_roi(sample_roi(:,2)>=overlap_rate,2)];%n=1105
Nsub_l_80 = [find(sample_roi(:,3)>=overlap_rate),sample_roi(sample_roi(:,3)>=overlap_rate,3)];%n=645
...
save AHBA_ROI_index_80 sample_roi Ncort_l_80 Nsub_l_80;
%%
...
save AHBA_exp_Mean_80ROI_raw_reanote Index_cortL Index_subL_use cort_expMeanRaw sub_expMeanRaw;
...
addpath(genpath('F:\work_dir\AHBAenrich\code'))
